...
def init_parsing_model(model_path, device="cpu"):
    ...
    # Check if it's a training checkpoint (has 'model_state_dict' key) or direct weights
    if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
        # This is a training checkpoint - extract just the model weights
        state_dict = checkpoint['model_state_dict']
        print("Loading model from training checkpoint...")
    else:
        # This is direct model weights (original format)
        state_dict = checkpoint
        print("Loading model weights directly...")
    
    # Load the weights (use strict=False to ignore LoRA-specific layers)
    ...
    if unexpected_keys:
        print(f"Warning: {len(unexpected_keys)} unexpected keys in checkpoint (likely LoRA layers)")
